...
void BER_RsPlusBs(int loop, int(*rs_bit)[N_dbps], int(*tbit)[N_dbps], int(*rbit)[N_dbps]){
	...
	printf("Eb/N0=%2d::loop=%6d:rs_BER = %1.2e :BER = %1.2e %1.2e \n", (int)CNR - 3 * MODULATION / 2, loop, rs_Pe, Pe, AverageBER);
	fprintf(output_log, "Eb/N0=%2d, loop=%6d, rs_BER = %1.2e, BER = %1.2e %1.2e \n", (int)CNR - 3 * MODULATION / 2, loop, rs_Pe, Pe, AverageBER);
	/*if (rs_Pe>0)getchar();
	if (Pe>0)getchar();*/

	if (loop == maxloop - 1){
		printf("Eb/No = %ld(dB) :: %f(dB)\tPER = %e\tAveBER = %e\n",
			(int)CNR - 3 * MODULATION / 2, 
			10.0*log10((signal_power) / (CODING_RATE*noise_power)) - 3.0,
