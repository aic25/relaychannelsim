...
double channel_gain[N][2], p_in[KI][N];

void MMSE_Weight_Matrix(double(*H)[CL][BS][2], double(*W)[KI][CL][2]){
	int rx, tx, tx2, f_index, i;
	double WH[N][KI][CL][2];
	double g_i[BS][2], g_iH[BS][2], power_i[2], g[TS][2], gH[TS][2], power[2], no;
	double HH[BS][CL][2], Rt[BS][BS][2], I[BS][BS][2], R[BS][BS][2];
	...
	for (f_index = 0; f_index < N; f_index++){
		...
		MatrixHermite_CLxCL(H[f_index], HH);	/* H^H = H^*T */
		MatrixMulMatrixToMatrix_CLxCL(HH, H[f_index], Rt);	/* Rt = H^H x H */
		MatrixAddMatrixToMatrix_CLxCL(Rt, I, Rt);	/* Rt = Rt + I */
		MatrixInverse_CLxCL(Rt);	/* Rt = (Rt + I)^-1 */
		MatrixMulMatrixToMatrix_CLxCLXCLxCL(Rt, HH, W[f_index]);	/* W = (Rt + I)^-1 x H^H */
		...
		MatrixMulMatrixToMatrix_CLxCL(WH[f_index], H[f_index], R);	/* R = (H^H x H + I)^-1 x H^H x H = (Rt + I)^-1 x Rt */
		/* -------------------------------------------- */
		...
		//g[1][0] = R[0][0][0];
		//g[1][1] = R[0][0][1];
		VectorConjugate_CL(g, gH);
		VectorMulVectorToScalar_CL(gH, g, power);
		...
		p_in[0][f_index] = (no)*power[0] + power_i[0];
#endif // SD
