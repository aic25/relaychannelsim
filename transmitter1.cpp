#include "const.h"
double B1_signal_power;
void transmitter1(int(*bit)[N_dbps], double(*transmitted_signal)[2]){
	...
	for (size_t k = 0; k < BURST; k++){
		B1_signal_power += sqr(transmitted_signal[k][0]) + sqr(transmitted_signal[k][1]);
	}
	B1_signal_power /= BURST;
}
...
void data_insert_trsym(int nd, int(*bit), double(*frequency_signal_out)[2]){
	int n, index;
	double bin2sgnlQPSK[2] = { -1.0, 1.0 };
	double bin2sgnlQAM[2][2] = { { -3.0, -1.0 }, { 3.0, 1.0 } };
	double bin2sgnl64QAM[2][2][2] = {
