...
void multipath_propagation(double(*transmitted_sigmat)[BS][BURST][2], double(*Received_Signal)[BURST][2], double CNR, double(*signal_power), double(*interference_power)){
	...
	for (ts = 0; ts < TS; ts++){
		for (bs = 0; bs < BS; bs++){
			for (time = 0; time < BURST; time++) {
				...
				if (bs == 0){ *signal_power += sqr(Received_Signal[ts][time][0]) + sqr(Received_Signal[ts][time][1]); }
				else { *interference_power += sqr(Received_Signal[ts][time][0]) + sqr(Received_Signal[ts][time][1]); }
			}
		}
	}
}
