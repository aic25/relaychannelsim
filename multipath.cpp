...
void multipath_propagation(double(*transmitted_sigmat)[BS][BURST][2], double(*received_signal)[BURST][2]){
	...
	for (ts = 0; ts < TS; ts++){
		for (bs = 0; bs < BS; bs++){
			for (time = 0; time < BURST; time++) {		
#ifdef PROPAGATION
				fading_process(time, w, (ts == 1) ? CNR : (CNR - (double)XI));	/* Most likely generates path coefficients. Time is the location on the long data stream. */
...
#else		  
				received_signal[ts][time][0] += w[0][0] * transmitted_sigmat[ts][bs][time][0] - w[0][1] * transmitted_sigmat[ts][bs][time][1];
				received_signal[ts][time][1] += w[0][1] * transmitted_sigmat[ts][bs][time][0] + w[0][0] * transmitted_sigmat[ts][bs][time][1];
#endif			
#else				   
