...
void multipath_propagation(double(*transmitted_sigmat)[BS][BURST][2], double(*received_signal)[BURST][2]){
	...
	double noise[2], noise_amplitude, norm_w = 0, norm_wr = 0, norm_wrb = 0;
		...
		if ((ts == 0) || (RS_DF == ON)){
			for (bs = 0; bs < BS; bs++){
				for (time = 0; time < BURST; time++) {
					...
					/*if (time == 0){
						norm_w = VectorNorm(w, PATH*DELAY);
						cout << "BS:" << bs << ", W in ts1: " << norm_w << endl;
					}  */
			...
			for (bs = 0; bs < BS; bs++){
				for (time = 0; time < BURST; time++) {
					...
					/*if (time == 0){
						norm_w = VectorNorm(w, PATH);
						norm_wr = VectorNorm(w_r, RS_PATH);
						cout << "BS:" << bs << ", W in ts2 : " << norm_w << endl;
						cout << "BS:" << bs << ", Wr in ts2: " << norm_wr << endl;
					}  */
				...
				/*	if (time == 0){
						norm_w = VectorNorm(w_extended, PATH*DELAY);
						norm_wr = VectorNorm(w_r_extended, RS_PATH*RS_DELAY);
						norm_wrb = VectorNorm(w_rb_extended, PATH*DELAY + RS_PATH*RS_DELAY - 1);
						cout << "BS:" << bs << ", We in ts2 : " << norm_w << endl;
						cout << "BS:" << bs << ", Wre in ts2: " << norm_wr << endl;
						cout << "BS:" << bs << ", Wrbe in ts2: " << norm_wrb << endl;
					}		   */
	...
	if (time == 0) {	/* Generate the channel at time instance 0, means constant over transmission duration. */
		...
		for (path_index = 0; path_index < PATH; path_index++){
			for (component_index = 0; component_index < RAY_COMPONENT; component_index++) {
				...
				a[path_index][component_index] = sqrt(power[path_index][component_index] * (((path_index == 0) && (component_index == 0) && (LOS == ON)) ? 1 : (-log(x))) * (pow(10.0, CNR / 10.0) / (double)CODING_RATE));
			}
		}
	}
	for (path_index = 0; path_index < PATH; path_index++) {
