	...
	if (time == 0) {	/* Generate the channel at time instance 0, means constant over transmission duration. */
		...
		for (path_index = 0; path_index < PATH; path_index++){
			for (component_index = 0; component_index < RAY_COMPONENT; component_index++) {
				...
				if (EXPMODEL == ON)	{ exponential_path(path_index, component_index, power); }
				else { samelevel_PATH(path_index, component_index, power); }
				x = (double)rand() / RAND_MAX;
				if (x < 1.0e-6) x = 1.0e-6;
				a[path_index][component_index] = sqrt(power[path_index][component_index] * -log(x) * (pow(10.0, CNR / 10.0) / CODING_RATE));
			}
		}
	}
	for (path_index = 0; path_index < PATH; path_index++) {
...
void exponential_path(int i, int kk, double(*power)[RAY_COMPONENT]){
	...
	for (path_index = 0; path_index < PATH; path_index++){
		...
		//path[path_index] = exp(-temp / (double)TAU);
		path[path_index] = (exp(-temp / (double)TAU) > pow(10, -6)) ? exp(-temp / (double)TAU) : 0;
		sum += path[path_index];
	}
