...
void noise_addition(double CNR, double(*input_signal)[2], double(*output_signal)[2], int count_for_power){
	int	time_index;
	...
	for (time_index = 0; time_index < BURST; time_index++) {
		noise_generator(noise_amplitude, noise, count_for_power);
...
void noise_generator(double n_amplitude, double(*noise), int count_for_power){
	...
	if (count_for_power == 1){ noise_power += sqr(noise[0]) + sqr(noise[1]); }
}
