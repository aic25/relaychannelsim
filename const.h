...
#define COMBINING				/* keep enabled */	
#define COMBINING2				/* not complete, keep enabled */
...
#define DELAY			5		/* Uniform delay between signal reflections */
...
/*----------------------------Relay Parameters----------------------------------------*/
#define SWITCH				OFF		/* ON - OFF :: OFF => relay re-transmits the exact received signal	*/		 
#define RLY_LOS
...
#define RLY_EXPMODEL		OFF
#define RS_PATH				2
#define RS_DELAY			5	
#define RS_RAY_COMPONENT	8								 
#define FDr					0.0		 
#define RS_DUR				10.0	/* ? Used in exponential distribution				*/
#define SIGMA_TAU			0.001		
...
void Relay_samelevel_PATH(int i, int kk, double(*power)[RS_RAY_COMPONENT]);		/* Normalize the fading coefficient																								*/
void Relay_exponential_PATH(int i, int kk, double(*power)[RS_RAY_COMPONENT]);	/* I guess used to calculate fading coefficient using exponential distribution													*/
...
/*--  BER.cpp  ----------------------------------------------------*/
void BER_RsPlusBs(int loop, int(*rsbit)[N_dbps], int(*tbit)[N_dbps], int(*rbit)[N_dbps]);
/*-------------------- RELAY 1 -------------------------------------*/
