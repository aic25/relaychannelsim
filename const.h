...
//#define W_OUTLOG
#ifdef W_OUTLOG
#define W_OUTPUT_TRESHOLD	2.5
#endif
//#define W_SELECTIVE
#ifdef W_SELECTIVE					   
#define W3
#define CORRECTION_TRESHOLD	0.001
#endif // W_SELECTIVE			   
#endif // DDWE	   			   
//#define ERR_POUT_PLOOP
//#define CORR_POUT_PLOOP
//#define SEPARATE_CORR_PLOOP
//#define RCV_VEC_AND_RCV_SIG_OUTPUT	/* Output part in ddwe */
#ifdef RCV_VEC_AND_RCV_SIG_OUTPUT
#define POUT_THOLD 0.3
#endif
...
void CoherentDetection4HD(int(*R_index), int data_number, double(*DFT_signal)[N][2], double(*WH)[BS][CL][2], int(*diLLR)[Nd][N_cbps], int(*Decision));
/*--  MMSE - weight.cpp  ------------------------------------------*/
void MMSE_Weight_Matrix(double(*)[TS][BS][2], double(*)[BS][TS][2]);
void MMSE_Weight_Compare(double(*)[TS][BS][2], double(*)[BS][TS][2]);
/*--  RLS.cpp  ----------------------------------------------------*/
void RLS_initial_weigth(int, int, double(*)[TS][N][2], double(*)[TS][N][2], double(*)[BS][CL][2], double(*)[TS][BS][2], double(*wh_sequence)[Np][N][2], double(*e)[NUMBER_OF_STEPS][2], double(*e_w)[NUMBER_OF_STEPS][2], double(*e_w3)[NUMBER_OF_STEPS][2]);
void DDWE_Update(int(*Decision), int R_index, double(*received_vector)[2], double(*received_vector_wonoise)[2], double(*reference_symbol), double(*old_weights)[2], double(*new_weights)[2], double(*H)[BS][2], double(*e)[NUMBER_OF_STEPS][2], double(*e_w)[NUMBER_OF_STEPS][2], double(*e_3)[NUMBER_OF_STEPS][2], double(*reference_symbol_bs3), int f_index, int);
...
/*--  receiver.cpp  -----------------------------------------------*/
void receiver(int(*_Rindex), double(*Received_Signal)[BURST][2], double(*Received_Signal_wonoise)[BURST][2], int(*rbit)[Nd][N_dbps], double(*wh_sequence)[Np][N][2]);
...
/*---------------Walsh-Hadamard--------------------------------------*/
void WHadamard_initialize(double(*)[BS][BURST][2], double(*)[Np][N][2]);
...
/*------jacobi------------------*/
#define M_Size 2
void eigsrt(double(*d), double(*v)[M_Size]);
void rot(double(*a)[M_Size], const double s, const double tau, const int i, const int j, const int k, const int l);
