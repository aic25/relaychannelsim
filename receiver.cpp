...
void receiver(double(*Received_Signal)[BURST][2], int(*rbit)[N_dbps], double(*wh_sequence)[Np][N]){
...
#endif // 			
	RLS_initial_weigth(0, DFT_signal, DFT_signal_ideal, W, H, wh_sequence);
#endif	// RLS
	/****************************** DETECTION ******************************/
#ifdef SD
	...
	for (index = Np; index < PACKETN; index++){
		...
		for (f_index = 0; f_index < N; f_index++){
			...
			DDWE_Update(received_vector, frequency_signal[f_index], W[f_index][0], W[f_index][0], H[f_index], e, Decision, f_index);
		}
#endif // DDWE
	}
#endif	    			
