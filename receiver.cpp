...
void receiver(double(*received_signal)[BURST][2], int(*rbit)[Nd][N_dbps]){
	int index, ts, k, f_index, coded_bit[KI][N_cbps], Decision[KI];
	...
	{
		R_index[f_index] = 0;
		RLS_initial_weigth(f_index, R_index[f_index], DFT_signal, DFT_signal_ideal, W, H);
	}
#endif	// RLS
	/****************************** DETECTION ******************************/
#ifdef SD
	...
	for (index = Np; index < PACKETN; index++){
		...
		for (f_index = 0; f_index < N; f_index++){
			...
			DDWE_Update(f_index, Decision, R_index[f_index], received_vector, 
				...
				W[f_index][R_index[f_index]], W[f_index][R_index[f_index]], H[f_index]); 
		}
#endif // DDWE
	}
#endif	    			
