...
#endif // 			
	RLS_initial_weigth(0, DFT_signal, DFT_signal_ideal, W, H, e_rls);
	if (loop == 0 && EbNo == EbNo_FROM)
	{
		if ((err = fopen_s(&mse, "mse.dat", "w+")) != 0)	{
			printf("The file mse.dat file was not opened\n");
			getchar();
			exit(1);
		};
		//printf("%d\t%d\n",loop,EbNo);
		for (size_t step = 0; step < Np; step++)
		{
			e_rls_av[step] = 0;
