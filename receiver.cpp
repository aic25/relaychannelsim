...
void receiver(double(*received_signal)[BURST][2], int(*rbit)[Nd][N_dbps]){
	...
	for (f_index = 0; f_index < N; f_index++){ 
		her[f_index][0][0] /= channel_power[f_index][0];
		her[f_index][0][1] /= channel_power[f_index][0];   
		her[f_index][1][0] /= channel_power[f_index][0];
		her[f_index][1][1] /= channel_power[f_index][0];
	}
