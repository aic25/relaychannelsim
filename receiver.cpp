...
void receiver(int(*R_index), double(*Received_Signal)[BURST][2], int(*rbit)[Nd][N_dbps], double(*wh_sequence)[Np][N]){
	FILE *mse;
	char strI[10];
	...
	double W[N][BS][CL][2];
	double e_rls[N][NUMBER_OF_STEPS][2], e_rls_av[NUMBER_OF_STEPS], temp[2], temp1[2];
	int Decision[KI];
#ifdef SD
	...
	for (f_index = 0; f_index < N; f_index++)
	{
		Rev_op = 0; Cor_op = 0;
		/*if (
			sqr(H[f_index][1][R_index[f_index]][0]) +
			sqr(H[f_index][1][R_index[f_index]][1]) <
			sqr(H[f_index][1][(R_index[f_index] + 1) % 2][0]) +
			sqr(H[f_index][1][(R_index[f_index] + 1) % 2][1]))
			{ R_index[f_index] = 1; }  */
		//R_index[f_index] = 1;
		RLS_initial_weigth(f_index, R_index[f_index], DFT_signal, DFT_signal_ideal, W, H, wh_sequence, e_rls);
	...
	cout << "Number of reversions: " << Rev_op << endl;
	cout << "Number of corrections: " << Cor_op <<endl;
#endif	// RLS
