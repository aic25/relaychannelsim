...
void receiver(double(*received_signal)[BURST][2], int(*rbit)[Nd][N_dbps]){
	int i, j, index, ts, k, f_index, coded_bit[KI][N_cbps], Decision[KI], Rev_op, Cor_op;
	...
	//double W[N][BS][CL][2];
	double e_rls[N][NUMBER_OF_STEPS][2], temp_RcvIAvg;
#ifdef SD
	...
	{
		R_index[f_index] = 0;
		RLS_initial_weigth(f_index, R_index[f_index], DFT_signal, DFT_signal_ideal, W, H, e_rls, 1);
	}
#else
#ifndef IW3ISELECTION
#ifndef IEISELECTION
	//Rev_op = 0; Cor_op = 0;
	for (f_index = 0; f_index < N; f_index++){
		R_index[f_index] = 0;
		RLS_initial_weigth(f_index, R_index[f_index], DFT_signal, DFT_signal_ideal, W, H, e_rls);
	...
	for (f_index = 0; f_index < N; f_index++){
		R_index[f_index] = 0;
		RLS_initial_weigth(f_index, R_index[f_index], DFT_signal, DFT_signal_ideal, W, H, e_rls);
		ScalarMulScalar_H(e_rls[f_index][PREAMBLE_LENGTH], e_rls[f_index][PREAMBLE_LENGTH], temp);
		R_index[f_index] = 1;
		RLS_initial_weigth(f_index, R_index[f_index], DFT_signal, DFT_signal_ideal, W, H, e_rls);
		ScalarMulScalar_H(e_rls[f_index][PREAMBLE_LENGTH], e_rls[f_index][PREAMBLE_LENGTH], temp1);
		if ((sqr(temp1[0]) + sqr(temp1[1])) >(sqr(temp[0]) + sqr(temp[1]))){
			R_index[f_index] = 0;
			RLS_initial_weigth(f_index, R_index[f_index], DFT_signal, DFT_signal_ideal, W, H, e_rls);
		}
	...
	//temp_RcvIAvg = 0;
	for (f_index = 0; f_index < N; f_index++){	
		temp[0] = temp[1] = temp1[0] = temp1[1] = 0;

		R_index[f_index] = 0;
		RLS_initial_weigth(f_index, R_index[f_index], DFT_signal, DFT_signal_ideal, W, H, e_rls, 1);
		RLS_initial_weigth(f_index, R_index[f_index], DFT_signal, DFT_signal_ideal, W, H, e_rls, 0);
		VectorMulVectorToScalarH_Np(e_rls[f_index], e_rls[f_index], temp); //temp[0] /= Np; temp[1] /= Np;
		/*for (i = 0; i < Np; i++) {
			temp[0] += e_rls[f_index][i][0] / Np;
			temp[1] += e_rls[f_index][i][1] / Np;
		}*/

		R_index[f_index] = 1;
		RLS_initial_weigth(f_index, R_index[f_index], DFT_signal, DFT_signal_ideal, W, H, e_rls, 1);
		RLS_initial_weigth(f_index, R_index[f_index], DFT_signal, DFT_signal_ideal, W, H, e_rls, 0);
		VectorMulVectorToScalarH_Np(e_rls[f_index], e_rls[f_index], temp1); //temp1[0] /= Np; temp1[1] /= Np;
		/*for (i = 0; i < Np; i++) {
			temp1[0] += e_rls[f_index][i][0] / Np;
			temp1[1] += e_rls[f_index][i][1] / Np;
		}*/
		...
		/*
		if (f_index == 5) {
			cout << "R index: " << R_index[f_index] << endl;
			cout << "|W_3| 0: " << temp[0] << "+j" << temp[1] << endl;
			cout << "|W_3| 1: " << temp1[0] << "+j" << temp1[1] << endl;
			getchar();
		}			*/
		//temp_RcvIAvg += R_index[f_index];
	}
	//cout << "Avg. Rvcr Index: " << temp_RcvIAvg / N << endl; // Check if receiver indexes are equiprobable.
#endif
#endif
	...
	for (index = Np; index < PACKETN; index++) {
		...
		for (f_index = 0; f_index < N; f_index++) {
			...
			DDWE_Update(f_index, Decision, R_index[f_index], received_vector,
			...
			/*DDWE_Update(f_index, Decision, R_index[f_index], received_vector,
				frequency_signal[0][f_index],
				frequency_signal[1][f_index],
				W[f_index][R_index[f_index]], W[f_index][R_index[f_index]], H[f_index], e_rls[f_index][index]);	   */
	}
#endif // DDWE
	}
