...
void RLS_initial_weigth(
	...
	for (n = 1; n < PREAMBLE_LENGTH + 1; n++){
		...
		x[TS][0] = wh_sequence[(receiver_index + 1) % 2][np][f_index][0];
		...
		VectorConjugate_CL(W[f_index][receiver_index], tempvech);
		VectorMulVectorToScalar_CL(x, tempvech, temp);
		...
		VectorMulVectorToScalarH_CL(W[f_index][receiver_index], W[f_index][receiver_index], e_w[f_index][n]);
		ScalarMulScalar_H(W[f_index][receiver_index][2], W[f_index][receiver_index][2], e_w3[f_index][n]);
