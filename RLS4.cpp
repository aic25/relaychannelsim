...
void RLS_datasym(
	...
	for (n = 1; n < PREAMBLE_LENGTH + 1; n++){
		np = n - 1;
		if ((f_index == 5) && (DBG == ON)) cout << "step: " << n << endl;
		for (ts = 0; ts < TS; ts++){
			y[ts][0] = DFT_signal[np][ts][f_index][0];
			y[ts][1] = DFT_signal[np][ts][f_index][1];
		}
		if ((f_index == 5) && (DBG == ON))cout << "y[0]: " << y[0][0] << " " << y[0][1] << endl;
		if ((f_index == 5) && (DBG == ON))cout << "y[1]: " << y[1][0] << " " << y[1][1] << endl;

		u[0] = y[1][0] -
			(
			H[f_index][1][0][0] * wh_sequence[receiver_index][np][f_index][0] -
			H[f_index][1][0][1] * wh_sequence[receiver_index][np][f_index][1]
			) -
			(
			H[f_index][1][1][0] * wh_sequence[(receiver_index + 1) % 2][np][f_index][0] -
			H[f_index][1][1][1] * wh_sequence[(receiver_index + 1) % 2][np][f_index][1]
			);
		u[1] = y[1][1] -
			(
			H[f_index][1][0][0] * wh_sequence[receiver_index][np][f_index][1] +
			H[f_index][1][0][1] * wh_sequence[receiver_index][np][f_index][0]
			) -
			(
			H[f_index][1][1][0] * wh_sequence[(receiver_index + 1) % 2][np][f_index][1] +
			H[f_index][1][1][1] * wh_sequence[(receiver_index + 1) % 2][np][f_index][0]
			);
		if ((f_index == 5) && (DBG == ON))cout << "u: " << u[0] << " " << u[1] << endl;
		d[0] = y[0][0] -
			(
			H[f_index][0][0][0] * wh_sequence[receiver_index][np][f_index][0] -
			H[f_index][0][0][1] * wh_sequence[receiver_index][np][f_index][1]
			) -
			(
			H[f_index][0][1][0] * wh_sequence[(receiver_index + 1) % 2][np][f_index][0] -
			H[f_index][0][1][1] * wh_sequence[(receiver_index + 1) % 2][np][f_index][1]
			);
		d[1] = y[0][1] -
			(
			H[f_index][0][0][0] * wh_sequence[receiver_index][np][f_index][1] +
			H[f_index][0][0][1] * wh_sequence[receiver_index][np][f_index][0]
			) -
			(
			H[f_index][0][1][0] * wh_sequence[(receiver_index + 1) % 2][np][f_index][1] +
			H[f_index][0][1][1] * wh_sequence[(receiver_index + 1) % 2][np][f_index][0]
			);
		...
		if ((f_index == 5) && (DBG == ON))cout << "e: " << e[f_index][np][0] << " " << e[f_index][np][1] << endl;
		if ((f_index == 5) && (DBG == ON))cout << "|e|^2: " << sqr(e[f_index][np][0])+ sqr(e[f_index][np][1]) << endl;
