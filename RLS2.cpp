...
void RLS_initial_weigth(
	...
	for (size_t f_index = 0; f_index < N; f_index++){
		for (n = 1; n < PREAMBLE_LENGTH + 1; n++){
			...
			MatrixMulVectorToVector_CLxCL(P_for_DDWE[f_index], x, k);  // eq. 1
			VectorConjugate_CL(k, kh);

			VectorMulVectorToScalar_CL(xh, k, temp);
			temp[0] += FORGETTING_FACTOR;
			temp2[0] = 1; temp2[1] = 0;
			ScalarDivScalarToScalar(temp2, temp, temp3);
			ScalarMulVectorToVector_CL(temp3, k, j);	// eq. 2

			VectorMulVectorToMatrix_CLxCL(k, kh, tempmat);
			VectorMulVectorToScalar_CL(xh, j, temp2);
			temp2[0] += FORGETTING_FACTOR;
			temp3[0] = 1; temp3[1] = 0;
			ScalarDivScalarToScalar(temp3, temp2, temp);
			ScalarMulMatrixToMatrix_CLxCL(temp, tempmat, tempmat2);
			MatrixSubMatrixToMatrix_CL(P_for_DDWE[f_index], tempmat2, tempmat3);
			temp3[0] = FF_INVERSE;
			ScalarMulMatrixToMatrix_CLxCL(temp3, tempmat3, P_for_DDWE[f_index]);	// eq. 3

			VectorMulVectorToScalar_CL(WH[f_index][receiver_index], x, temp);
			/* for (ts = 0; ts < CL; ts++)
			{
			temp[0] += W[f_index][0][ts][0] * x[ts][0] - W[f_index][0][ts][1] * x[ts][1];
			temp[1] += W[f_index][0][ts][0] * x[ts][1] + W[f_index][0][ts][1] * x[ts][0];
			}	*/
			if (CL == 3){
			...
			VectorConjugate_CL(tempvec2, WH[f_index][receiver_index]);
			VectorConjugate_CL(WH[f_index][receiver_index], W[f_index][receiver_index]);
		}	/* end :: n */
