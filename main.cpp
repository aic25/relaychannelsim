...
void main(){
	...
	for (EbNo = EbNo_FROM; EbNo >= EbNo_TO; EbNo -= 5.0){	/* begin :: EbNo */
		...
		for (loop = 0; loop < maxloop; loop++){	/* begin :: loop */
			...
			for ( ts = 0; ts < TS; ts++)
			{
				noise_addition(CNR, transmitted_sigvec[ts], transmitted_sigvec[ts], &noise_power);	/* Add noise to first? time slot */
			}
#endif // NOISE
			receiver(R_index, transmitted_sigvec, received_bit, wh_sequence);
