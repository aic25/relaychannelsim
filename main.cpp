...
void main(){
	...
	for (EbNo = EbNo_FROM; EbNo >= EbNo_TO; EbNo -= 5.0){	/* begin :: EbNo */
		RLY_CNR = (double)EbNo + 3.0 * MODULATION / 2.0;
#ifdef ADPTV_LOOP
		maxloop = 10*intPow((int)EbNo, 2) + 10;
#else
		maxloop = LOOPN;
#endif // ADPTV_LOOP
		for (loop = 0; loop < maxloop; loop++){	/* begin :: loop */
			for (size_t rs = 0; rs < 1; rs++)
			{
				bs = rs;
			...
			{
#ifdef PROPAGATION
			//	noise_addition(0, transmitted_sigvec[ts], transmitted_sigvec[ts]);	/* Add noise to first? time slot. CNR=0 bcs it is included in propogation */
#else
			//	noise_addition((ts == 1) ? CNR : (CNR - LOSS), transmitted_sigvec[ts], transmitted_sigvec[ts]);	/* Add noise to first? time slot. CNR=0 bcs it is included in propogation */
#endif
			}
#endif // NOISE
			//receiver(transmitted_sigvec, received_bit);
			//BER_RsPlusBs(loop, rs_bit[0], transmitted_bit[0], received_bit);
			BER(loop, transmitted_bit[0], rs_bit[0]);
		}	/* end :: loop */
	}	/* end :: EbNo */
}	/* end :: main() */
