...
double e_rls[N][NUMBER_OF_STEPS][2], e_rls_avg[NUMBER_OF_STEPS];
double e_w[N][NUMBER_OF_STEPS][2], e_w_avg[NUMBER_OF_STEPS];
double e_w3[N][NUMBER_OF_STEPS][2], e_w3_avg[NUMBER_OF_STEPS];
double Pe;


void main(){
	double noise_power, MS_signal_power, MS_interference_power;
	...
	double transmitted_sigvec_withnoise[TS][BURST][2];
	double BS_signal_power[BS];
	int transmitted_bit[BS][Nd][N_dbps];
	int received_bit[KI][Nd][N_dbps];
	int bit[BS][Nd][N_dbps];
	int R_index[N];
	int n, rs, bs, ts;
	...
	for (EbNo = EbNo_FROM; EbNo >= EbNo_TO; EbNo -= 5.0){	/* begin :: EbNo */
		...
		//maxloop = 20;
		//maxloop = 8 * (int)EbNo + 10;
#else
		maxloop = LOOPN;
#endif // ADPTV_LOOP
		for (loop = 0; loop < maxloop; loop++){	/* begin :: loop */
			for (n = 0; n < N; n++)	{ R_index[n] = 0; }
			initialization(&noise_power, &MS_signal_power, &MS_interference_power);
			for (rs = 0; rs < BS; rs++)
			{
				bs = rs;
				transmitter(transmitted_bit[bs], transmitted_signal[bs], bs, BS_signal_power);
				relay(transmitted_signal[rs], relay_tx_signal[rs], bit[rs], rs, BS_signal_power);	/* I see no difference */
			}
			transmitted_signal_Matrix(transmitted_signal, relay_tx_signal, transmitted_sigmat);	/* Combine those for into the fifth */
			multipath_propagation(transmitted_sigmat, transmitted_sigvec, 0, &MS_signal_power, &MS_interference_power);	/* Signal propogates thru channel. We also need the signal which gone thru an ideal channel*/
#ifdef NOISE
			for (ts = 0; ts < TS; ts++)
			{
				noise_addition(CNR, transmitted_sigvec[ts], transmitted_sigvec_withnoise[ts], &noise_power);	/* Add noise to first? time slot */
			}
#endif // NOISE
			receiver(R_index, transmitted_sigvec_withnoise, transmitted_sigvec, received_bit, wh_sequence);
			BER(R_index, loop, maxloop, CNR, transmitted_bit, received_bit, noise_power, MS_signal_power, MS_interference_power);
		}	/* end :: loop */
	}	/* end :: EbNo */
}	/* end :: main() */