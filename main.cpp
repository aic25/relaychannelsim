...
void main() {
	...
	int n, bs, ts, rs, rs_bit[BS][Nd][N_dbps];
...
#endif // WH
	/*~~~~~~~~initialize end~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	for (EbNo = EbNo_FROM; EbNo >= EbNo_TO; EbNo -= 3.0) {	/* begin :: EbNo */
		initialization();
		CNR = (double)EbNo + 3.0 * MODULATION / 2.0;
#ifdef ADPTV_LOOP
		maxloop = (EbNo > 0 ? (CHANNEL_ENCODING ? (2 * (512 / N)*(intPow((int)EbNo, 2)) + 10) : (2*(intPow((int)EbNo, 2)) + 10)) : 10);
		//maxloop = 5;
#else
		maxloop = LOOPN;
#endif // ADPTV_LOOP
...
#if THEORETICAL == ON
			/*new function that takes W, H, CNR, RLY CNR and calculates theoretical BER. It needs to average it over H and all loops/ Inputs: loop, H, W, CNR, RLY_CNR, LOSS*/
			Theoretical_BER(loop, H, W);
#else 
			BER_RsPlusBs(loop, rs_bit, transmitted_bit, received_bit);
#endif								 
		}	/* end :: loop */
	}	/* end :: EbNo */
}	/* end :: main() */