...
int loop;
int maxloop;
/* Rx : time slot index, Tx is for BS when Rx = 0, and it is for relay when Rx = 1. */
double wh_sequence[2][Np][N];
double transmitted_sigmat[Rx][Tx][BURST][2];	/* This combines all transmitted singal into matrix form. */
...
void main(){
	...
	double transmitted_sigvec[Rx][BURST][2];	/* Received signal at MS1 after multipath propogation. */
