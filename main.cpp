...
void main(){
	...
	for (EbNo = EbNo_FROM; EbNo >= EbNo_TO; EbNo -= 5.0){	/* begin :: EbNo */
		...
		for (loop = 0; loop < maxloop; loop++){	/* begin :: loop */
			...
			multipath_propagation(transmitted_sigmat, transmitted_sigvec, CNR, &MS_signal_power, &MS_interference_power);	/* Signal propogates thru channel. We also need the signal which gone thru an ideal channel*/
#ifdef NOISE
			for (size_t ts = 0; ts < TS; ts++)
			{
				noise_addition(0, transmitted_sigvec[ts], transmitted_sigvec[ts], &noise_power);	/* Add noise to first? time slot */
			}
#endif // NOISE
