...
void init_channel_models(){
	...
	for (path_index = 0; path_index < RS_PATH; path_index++){
		for (component_index = 0; component_index < ((path_index == 0 && RLY_LOS == ON) ? 1 : RS_RAY_COMPONENT); component_index++){
...
#endif
	fprintf(fdata, "# Preamble length(Correlation length): %d\n", PREAMBLE_LENGTH);
	printf("# Preamble length(Correlation length): %d\n", PREAMBLE_LENGTH);

	fprintf(fdata, "# Relay EbN0: Fixed to %d dB, %d dB \n", RLY_CNR, (RLY_CNR - 3 * MODULATION / 2));
	printf("# Relay EbN0: Fixed to %d dB, %d dB \n", RLY_CNR, (RLY_CNR - (3 * MODULATION / 2)));
...
#ifndef DDWE
#ifndef EVDF
	fprintf(fdata, "# Combining: Eig Vec.\n");
	printf("# Combining: Eig Vec\n");
#else							  
	fprintf(fdata, "# Combining: Eig Vec. with DF\n");
	printf("# Combining: Eig Vec with DF\n");
#endif // !EVDF

#else
	fprintf(fdata, "# Combining: ERROR!(DDWE)\n");
	printf("# Combining: DDWE\n");
#endif // !DDWE	
#endif	 
	fprintf(fdata, "# Metric Order: %d\n", Metric_Order);
	printf("# Metric Order: %d\n", Metric_Order);
