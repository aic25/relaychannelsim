...
#ifdef RLY_LOS
		K = Relay_LOS_exponentialPATH(h_r[bs]);
		/*for (i = 0; i < 2; i++){
			for (j = 0; j < RS_PATH; j++){
	...
	if (time == 0) {
		...
		for (path_index = 0; path_index < RS_PATH; path_index++){
			for (kk = 0; kk < RS_RAY_COMPONENT; kk++) {
				...
				a[path_index][kk] = sqrt(power[path_index][kk] * (((path_index == 0) && (RLY_LOS2 == ON)) ? 1 : (-log(x))) * (pow(10.0, 0 / 10.0) / CODING_RATE));
			}
		}
	}
	for (path_index = 0; path_index < RS_PATH; path_index++) {
...
double Relay_LOS_exponentialPATH(double(*h)[2]){
	int path_index;
	double sum, K, temp[2];
	double path[RS_PATH];
	...
	for (path_index = 0; path_index < RS_PATH; path_index++)
	{
		...
		path[path_index] = exp((-RS_DUR * log(10.0) * ((double)path_index)) / (5.0 * (double)(RS_PATH - 1)));
