...
#else																			  
	// cout << "tx sig at relay: " << relay_in[0][0] << " " << relay_in[0][1] << endl;
...
void Relay_AF(double(*relay_in)[2], double(*relay_out)[2], int(*bit)[N_dbps], int bs){
...
#ifdef RLY_PROPOGATION		 
	// cout << "tx sig at relay_af: " << relay_in[0][0] << " " << relay_in[0][1] << endl;
	...
	Relay_receiver(relay_out, bit, bs);	/* HD decoder. */
}
...
#ifdef RLY_LOS
		K = Relay_LOS_exponentialPATH(path, h_r[bs]);
		/*for (i = 0; i < 2; i++){
			for (j = 0; j < RS_PATH; j++){
				h_r[i][j][0] = 1; h_r[i][j][1] = 0;
			}
		} */
#endif
	...
	if (time == 0) {
		...
		for (path_index = 0; path_index < RS_PATH; path_index++){
			for (kk = 0; kk < RS_RAY_COMPONENT; kk++) {
				...
				a[path_index][kk] = sqrt(power[path_index][kk] * -log(x) * (pow(10.0, 0 / 10.0) / CODING_RATE));
			}
		}
	}
	for (path_index = 0; path_index < RS_PATH; path_index++) {
...
void Relay_exponential_PATH(int i, int kk, double(*power)[RS_RAY_COMPONENT]){
	...
	sum = 0.0;
	for (path_index = 0; path_index < RS_PATH; path_index++){
		path[path_index] = exp((-RS_DUR * log(10.0) * ((double)path_index)) / (10.0 * (double)(RS_PATH - 1)));
		sum += path[path_index];
	}
	power[i][kk] = (double)path[i] / (((double)RS_RAY_COMPONENT) * sum);
}

double Relay_LOS_exponentialPATH(double(*path), double(*h)[2]){
	int path_index;
	double sum, K, temp[2];
	...
	for (path_index = 0; path_index < RS_PATH; path_index++)
	{
		path[path_index] = exp((-(path_index) * RS_DELAY * (1 / SAMPLING_RATE)) / (SIGMA_TAU));
		h[path_index][0] *= path[path_index];
		h[path_index][1] *= path[path_index];
		//sum += path[path_index];
		sum += sqr(h[path_index][0]) + sqr(h[path_index][1]);
	}
	K = 1 / (sum-1);
	for (path_index = 0; path_index < RS_PATH; path_index++){
		h[path_index][0] /= 2;
	...
	//power[isi_path][ray_component] = (double)path[isi_path] / (((double)RS_RAY_COMPONENT) * sum);
	return K;
