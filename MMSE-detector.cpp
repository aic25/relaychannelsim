...
void CoherentDetection4HD(
	...
	for (n = 0; n < N; n++) {
		...
		for (ts = 0; ts < TS; ts++) {
			...
			if (Metric_Order>1)
			{
				temp1[0] += +W[n][(R_index[n] + 1) % 2][ts][0] * DFT_signal[ts][n][0] + W[n][(R_index[n] + 1) % 2][ts][1] * DFT_signal[ts][n][1];
				temp1[1] += +W[n][(R_index[n] + 1) % 2][ts][0] * DFT_signal[ts][n][1] - W[n][(R_index[n] + 1) % 2][ts][1] * DFT_signal[ts][n][0];
				if (Metric_Order > 2)
				{
					temp2[0] += +W[n][2][ts][0] * DFT_signal[ts][n][0] + W[n][2][ts][1] * DFT_signal[ts][n][1];
					temp2[1] += +W[n][2][ts][0] * DFT_signal[ts][n][1] - W[n][2][ts][1] * DFT_signal[ts][n][0];
					if (Metric_Order > 3)
					{
						temp3[0] += +W[n][3][ts][0] * DFT_signal[ts][n][0] + W[n][3][ts][1] * DFT_signal[ts][n][1];
						temp3[1] += +W[n][3][ts][0] * DFT_signal[ts][n][1] - W[n][3][ts][1] * DFT_signal[ts][n][0];
					}
				}
			}
		}
		...
		if (MODULATION == QPSK){
			...
			for (m1 = 0; m1 < 4; m1++){
				for (m3 = 0; m3 < 4; m3++){
					...
					if (Metric_Order > 1)	{
						e_1[0] = temp1[0] + (+W[n][(R_index[n] + 1) % 2][2][0] * s_qpsk[m3][0] + W[n][(R_index[n] + 1) % 2][2][1] * s_qpsk[m3][1]) + (+W[n][(R_index[n] + 1) % 2][3][0] * s_qpsk[m1][0] + W[n][(R_index[n] + 1) % 2][3][1] * s_qpsk[m1][1]);
						e_1[1] = temp1[1] + (+W[n][(R_index[n] + 1) % 2][2][0] * s_qpsk[m3][1] - W[n][(R_index[n] + 1) % 2][2][1] * s_qpsk[m3][0]) + (+W[n][(R_index[n] + 1) % 2][3][0] * s_qpsk[m1][1] - W[n][(R_index[n] + 1) % 2][3][1] * s_qpsk[m1][0]);
						e_1[0] = (sqr(e_1[0]) + sqr(e_1[1])) / EigSort[icerik[(R_index[n] + 1) % 2]];
						e[1][0] += e_1[0];
						if (Metric_Order > 2){
							e_2[0] = temp2[0] + (+W[n][2][2][0] * s_qpsk[m3][0] + W[n][2][2][1] * s_qpsk[m3][1]) + (+W[n][2][3][0] * s_qpsk[m1][0] + W[n][2][3][1] * s_qpsk[m1][1]);
							e_2[1] = temp2[1] + (+W[n][2][2][0] * s_qpsk[m3][1] - W[n][2][2][1] * s_qpsk[m3][0]) + (+W[n][2][3][0] * s_qpsk[m1][1] - W[n][2][3][1] * s_qpsk[m1][0]);
							e_2[0] = (sqr(e_2[0]) + sqr(e_2[1])) / EigSort[icerik[2]];
							e[1][0] += e_2[0];
							if (Metric_Order > 3){
								e_3[0] = temp3[0] + (+W[n][3][2][0] * s_qpsk[m3][0] + W[n][3][2][1] * s_qpsk[m3][1]) + (+W[n][3][3][0] * s_qpsk[m1][0] + W[n][3][3][1] * s_qpsk[m1][1]);
								e_3[1] = temp3[1] + (+W[n][3][2][0] * s_qpsk[m3][1] - W[n][3][2][1] * s_qpsk[m3][0]) + (+W[n][3][3][0] * s_qpsk[m1][1] - W[n][3][3][1] * s_qpsk[m1][0]);
