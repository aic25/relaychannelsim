...
void CoherentDetection4HD(
	...
	int i, ts, n, index, detected_signal[KI][N_cbps], s3_index, s1_index, m1, m3;
	double
		temp[2],
		temp1[2],
		e[2][2],
	...
	for (n = 0; n < N; n++) {
		...
		//cout << complex<double>(W[n][0][0][0], -W[n][0][0][1]) << endl; getchar();
	/*MMSE bypass*//*
		double hbhrH3[2], hbhbH3[2], mmse_w[2];
		ScalarMulScalar_H(H[n][0][2], H[n][1][2], hbhrH3);
		ScalarMulScalar_H(H[n][0][2], H[n][0][2], hbhbH3);
		hbhbH3[0] += pow(10.0, -CNR / 10.0) / CODING_RATE;
		ScalarDivScalarToScalar(hbhrH3, hbhbH3, mmse_w);
		W[n][R_index[n]][0][0] = -W[n][R_index[n]][0][0];
		W[n][R_index[n]][0][1] = -W[n][R_index[n]][0][1];
		MMSE bypass end*/
		...
		if (MODULATION == QPSK) {
			...
			for (m1 = 0; m1 < 4; m1++) {
				for (m3 = 0; m3 < 4; m3++) {
					...
					if (S_index[n] == 0) {
						u[0] = -(y[1][0] -
							(H[n][1][R_index[n]][0] * s_qpsk[m1][0] - H[n][1][R_index[n]][1] * s_qpsk[m1][1]) -
							(H[n][1][(R_index[n] + 1) % 2][0] * s_qpsk[m3][0] - H[n][1][(R_index[n] + 1) % 2][1] * s_qpsk[m3][1]));
						u[1] = -(y[1][1] -
							(H[n][1][R_index[n]][0] * s_qpsk[m1][1] + H[n][1][R_index[n]][1] * s_qpsk[m1][0]) -
							(H[n][1][(R_index[n] + 1) % 2][0] * s_qpsk[m3][1] + H[n][1][(R_index[n] + 1) % 2][1] * s_qpsk[m3][0]));

						d[0] = y[0][0] -
							(H[n][0][R_index[n]][0] * s_qpsk[m1][0] - H[n][0][R_index[n]][1] * s_qpsk[m1][1]) -
							(H[n][0][(R_index[n] + 1) % 2][0] * s_qpsk[m3][0] - H[n][0][(R_index[n] + 1) % 2][1] * s_qpsk[m3][1]);
						d[1] = y[0][1] -
							(H[n][0][R_index[n]][0] * s_qpsk[m1][1] + H[n][0][R_index[n]][1] * s_qpsk[m1][0]) -
							(H[n][0][(R_index[n] + 1) % 2][0] * s_qpsk[m3][1] + H[n][0][(R_index[n] + 1) % 2][1] * s_qpsk[m3][0]);
					}
					else {
						d[0] = y[1][0] -
							(H[n][1][R_index[n]][0] * s_qpsk[m1][0] - H[n][1][R_index[n]][1] * s_qpsk[m1][1]) -
					...
					wu = complex<double>(W[n][R_index[n]][0][0], -W[n][R_index[n]][0][1])*complex<double>(u[0], u[1]);
					e[1][0] = d[0] - wu.real();
					e[1][1] = d[1] - wu.imag();

					/*cout << "hr - w*hb:"
						<< pow(abs(complex<double>(H[n][1][2][0], H[n][1][2][1])
						- complex<double>(W[n][0][0][0], -W[n][0][0][1])
						*complex<double>(H[n][0][2][0], H[n][0][2][1])), 2) << endl;
					cout << "hb - w*hr:"
						<< pow(abs(complex<double>(H[n][0][2][0], H[n][0][2][1])
						- complex<double>(W[n][0][0][0], -W[n][0][0][1])
						*complex<double>(H[n][1][2][0], H[n][1][2][1])), 2) << endl;
					cout << "hr + w*hb:"
						<< pow(abs(complex<double>(H[n][1][2][0], H[n][1][2][1])
						+ complex<double>(W[n][0][0][0], -W[n][0][0][1])
						*complex<double>(H[n][0][2][0], H[n][0][2][1])), 2) << endl;
					...
					/*double wu[2];
					ScalarMulScalar_H(W[n][R_index[n]][0], u, wu);
					//ScalarMulScalar_H(u, W[n][R_index[n]][0], wu);
					e[1][0] = d[0] - wu[0];
					e[1][1] = d[1] - wu[1];				  */
					if (m1 == 0 && m3 == 0) {
						e[0][0] = e[1][0];
