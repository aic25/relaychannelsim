...
void CoherentDetection4HD(
	...
	double e[2][2], temp1[2], chpower[N];
	int s2_index, s1_index, m1, m2;
	...
	{
		chpower[f_index] = ((sqr(H[f_index][0][0][0]) + sqr(H[f_index][0][0][1])) + (sqr(H[f_index][0][1][0]) + sqr(H[f_index][0][1][1])));
	}
	index = 0;
	...
	for (n = 0; n < N; n++) {
		...
		if (MODULATION == QPSK){
			...
			{
				for (size_t m2 = 0; m2 < 4; m2++)
				{
					e[1][0] = ((H[n][0][0][0] * s_qpsk[m1][0] - H[n][0][0][1] * s_qpsk[m1][1])
						+ (H[n][0][1][0] * s_qpsk[m2][0] - H[n][0][1][1] * s_qpsk[m2][1])) - temp[0];
					e[1][1] = (((H[n][0][0][0] * s_qpsk[m1][1] + H[n][0][0][1] * s_qpsk[m1][0])
						+ (H[n][0][1][0] * s_qpsk[m2][1] + H[n][0][1][1] * s_qpsk[m2][0])) - temp[1]);
					if (m1 == 0 && m2 == 0){
						e[0][0] = e[1][0];
						e[0][1] = e[1][1];
						s1_index = 0;
			...
			detected_signal[index] = (s_qpsk[s1_index][0] >= 0.0) ? 1 : 0;
			detected_signal[index + 1] = (s_qpsk[s1_index][1] >= 0.0) ? 1 : 0;
#else
#ifdef PROPAGATION
