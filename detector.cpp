#include "const.h"
extern double channel_gain[N][2], p_in[N], her[N][TS][2], channel_power[N][2], H[N][TS][BS][2];
extern int loop;
		...
		if (MODULATION == QPSK){
			...
			for (m1 = 0; m1 < 4; m1++){
				for (m3 = 0; m3 < 4; m3++){
					...
					temp1[1] =
						(H[n][0][0][1] * s_qpsk[m1][0] + H[n][0][0][0] * s_qpsk[m1][1]) +
						(H[n][0][1][1] * s_qpsk[m3][0] + H[n][0][1][0] * s_qpsk[m3][1]);
					e[1][0] = temp1[0] - temp[0];
					e[1][1] = temp1[1] - temp[1];
					//cout << m1 << m3 << "\n";
