...
void WH_channel_estimation(double(*DFT_signal)[TS][N][2], double(*H)[TS][BS][2], double(*wh_sequence)[Np][N][2]){
	double temp[2];
	int tx, rx, f_index, s_index;
	temp[0] = temp[1] = 0;
	for (tx = 0; tx < BS; tx++){
		for (rx = 0; rx < TS; rx++){
			for (f_index = 0; f_index < N; f_index++){
				whsum[f_index][rx][tx] = 0;
				for (s_index = 0; s_index < Np; s_index++){	/* This 0 and 1 below is for Q-I */
					...
					whsum[f_index][rx][tx] += sqr(wh_sequence[tx][s_index][f_index][0]) + sqr(wh_sequence[tx][s_index][f_index][1]);
				}
				H[f_index][rx][tx][0] = temp[0] / whsum[f_index][rx][tx];
				H[f_index][rx][tx][1] = temp[1] / whsum[f_index][rx][tx];
				temp[0] = 0; temp[1] = 0;
			}
		}
	}
...
void WH_chest_update(double(*DFT_signal)[2], double(*H)[BS][2], double detected_signal[2], int f_index){
	...
	for (tx = 0; tx < BS; tx++){
		for (rx = 0; rx < TS; rx++){
			H[rx][tx][0] *= whsum[f_index][rx][tx];
			H[rx][tx][1] *= whsum[f_index][rx][tx];
			H[rx][tx][0] += DFT_signal[rx][0] * detected_signal[0] + DFT_signal[rx][1] * detected_signal[1];
			H[rx][tx][1] += DFT_signal[rx][1] * detected_signal[0] - DFT_signal[rx][0] * detected_signal[1];
			whsum[f_index][rx][tx] += sqr(detected_signal[0]) + sqr(detected_signal[1]);
			H[rx][tx][0] /= whsum[f_index][rx][tx];
			H[rx][tx][1] /= whsum[f_index][rx][tx];
		}
	}
