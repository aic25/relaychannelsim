...
void WHadamard_initialize(double(*transmitted_signal)[Tx][BURST][2], double(*wh_sequence)[Np][N]){
	...
	int WH_correlation[Np][Np][Np], WH_correlation_sum[Np][Np];
	int code[Np][Np];
	int h_init, v_init, h, v, Pow, retrial, code1, code2, m, n;
	int code_distance[Np][Np], code_distance_2[Np][Np], code_distance_4[Np][Np], code_distance_8[Np][Np];
...
void WH_channel_estimation(double(*DFT_signal)[Rx][N][2], double(*H)[Rx][Tx][2], double(*wh_sequence)[Np][N]){
	double temp[2];
	temp[0] = temp[1] = 0;
	for (int tx = 0; tx < Tx; tx++){
		for (int rx = 0; rx < Rx; rx++){
			for (int f_index = 0; f_index < N; f_index++){
				for (int s_index = 0; s_index < Np; s_index++){
				...
				H[f_index][rx][tx][0] = temp[0] / Np;
				H[f_index][rx][tx][1] = temp[1] / Np;
				temp[0] = 0; temp[1] = 0;
			}
		}
	}
