...
void RLS_initial_weigth(
	...
	for (size_t f_index = 0; f_index < N; f_index++){
...
#ifdef SD_NORMALIZE
		//MatrixHermite_TSxBS(H[f_index], HH);
		//MatrixMulMatrixToMatrix_BSxTS(HH, H[f_index], HHR);
		...
		//channel_gain[f_index][0] = HHR[0][0][0];
		//channel_gain[f_index][1] = -HHR[0][0][1];		 
		//channel_gain[f_index][0] = HH[0][0][0];
		//channel_gain[f_index][1] = -HH[0][0][1];
		//channel_gain[f_index][0] = 1.0;
		//channel_gain[f_index][1] = 0.0;

		//p_in[f_index] = (no)*power[0] + power_i[0];
		p_in[f_index] = (no_loss)*power[0] + (no)*power[1] + power_i[0];	
		//p_in[f_index] = (no)*power[0] + (no)*power[1] + power_i[0];
		//p_in[f_index] = 1;
		//p_in[f_index] = (no_loss)+(no)+(sqr(HH[0][0][0]) + sqr(HH[0][0][1]));
#endif // SD
