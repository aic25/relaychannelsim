...
void transmitter(int(*bit)[N_dbps], double(*transmitted_signal)[2], int bs){
	...
	for (k = 0; k < Nd; k++){
		...
		else for (i = 0; i < N_dbps; i++) { coded_bit[k][i] = bit[k][i]; }
	}

	for (nd = 0; nd < Nd; nd++){
		data_insert(nd, coded_bit, frequency_signal);
		(Fourier_MOD == ON) ? IFFT(frequency_signal[nd], time_signal[nd]) : IDFT(frequency_signal[nd], time_signal[nd]);
	}
...
void info_generator(int(*data_bit)){
	/* Information Generator
		Generate one and zeros as data sequence.
		Usage: info_generator(<Pointer to N_dbps length>)
		*/
	int n, i;
	double number;

	for (n = 0; n < (CHANNEL_ENCODING ? (N_dbps - TAIL) : N_dbps); n++) {
		number = (double)rand() / RAND_MAX;
		data_bit[n] = (number>0.5) ? 1 : 0;
	}
	if (CHANNEL_ENCODING)	for (n = 0; n < TAIL; n++) data_bit[N_dbps - TAIL + n] = 0;
}
