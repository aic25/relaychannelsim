...
void transmitter(int(*bit)[N_dbps], double(*transmitted_signal)[2], int bs){
	...
	for (k = 0; k < Nd; k++){
		...
		// if(k==0)cout << "bit: " << bit[0][0] << " " << bit[0][1] << endl;
		convolutional_encoder(bit[k], coded_bit[k]);	/* k is the symbol index */	
		// if (k == 0)cout << "coded bit: " << coded_bit[0][0] << " " << coded_bit[0][1] << endl;
		interleaver(coded_bit[k], coded_bit[k]);
		// if (k == 0)cout << "interleaved bit: " << coded_bit[0][0] << " " << coded_bit[0][1] << endl;
	}

	for (nd = 0; nd < Nd; nd++){
		data_insert(nd, coded_bit, frequency_signal);	/* Modulate bits to symbols for each packet */
		// if (nd == 0)cout << "freq signal: " << frequency_signal[0][0][0] << " " << frequency_signal[0][0][1] << endl;
		IFFT(frequency_signal[nd], time_signal[nd]);	/* Generate OFDM symbol of index nd */			  
		// if (nd == 0)cout << "time signal: " << time_signal[0][0][0] << " " << time_signal[0][0][1] << endl;
	}

	for (nd = 0; nd < Nd; nd++){
		data_GI_insert(time_signal[nd], data_signal);
		// if (nd == 0)cout << "data signal: " << data_signal[0][0] << " " << data_signal[0][1] << endl;
		for (t_index = 0; t_index < SAMPLEN; t_index++){
			...
			// if ((t_index == 0) && (nd == 0))cout << "time: " << time << endl << "t_index: " << t_index << endl;
			// if ((t_index == 0) && (nd == 0))cout << "transmitted signal: " << transmitted_signal[time][0] << " " << transmitted_signal[time][1] << endl;
		}
	}
...
void data_insert_regenerated(int nd, int(*bit), double(*frequency_signal_out)[2]){
	...
	for (size_t carrier_index = 0; carrier_index < N; carrier_index++) {
